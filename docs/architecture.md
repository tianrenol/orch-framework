...
* [Orch-Framework 架构设计与技术文档](#orch-framework-架构设计与技术文档)
  ...
  * [六、管道编排引擎详解](#六管道编排引擎详解)
    ...
    * [6.3 StepOutputKey - 类型安全的步骤输出键](#63-stepoutputkey---类型安全的步骤输出键)
    * [6.4 StepOutputStore - 步骤输出存储](#64-stepoutputstore---步骤输出存储)
    * [6.5 多模块编排](#65-多模块编排)
    * [6.6 管道执行生命周期](#66-管道执行生命周期)
  * [七、异常体系设计](#七异常体系设计)
...
BusinessFacade<V, T, R>              (模板方法入口)
    ...
    │       └── 8. publish(provides, V)    发布步骤输出
    │
    ├── requires() : Set<StepOutputKey>  (声明前置依赖)
    ├── provides() : StepOutputKey       (声明产出输出)
    │
    └── 消费方实现: OrderFacade, PaymentFacade, ...

StepOutputKey<T>                      (类型安全的步骤输出键)
    ├── name: String                  唯一标识
    ├── type: Class<T>               值类型（运行时类型转换）
    └── of(name, type): 工厂方法

StepOutputStore                       (类型安全的异构容器)
    ├── publish(key, value)           发布步骤输出（禁止重复）
    ├── require(key): T              获取输出（不存在则抛异常）
    └── get(key): T                  获取输出（不存在返回 null）
...
Consumer        Facade          Assembly        Helper          Entity          Env
  ...
  │               │               │               │──executeFutureTasks()         │
  │               │               │               │               │               │
  │               │──publish(provides, dealVO)───>│  (步骤输出发布到OutputStore)  │
  │               │               │               │               │               │
  │<──dealVO──────│               │               │               │               │
```
...
orch-framework-core (纯 Java，零框架依赖)
    ...
    │   ├── BusinessFacade
    │   ├── StepOutputKey          ← 类型安全的步骤输出键
    │   ├── StepOutputStore        ← 步骤输出存储（异构容器）
    │   └── StepRegistration       ← 步骤注册信息（框架内部）
    │
    ...
    R extends UserBusinessVO>            // 输入类型
{
    ...
    public V process(R r) {
        ...
        // 9. publish(provides, v) → 自动发布步骤输出
        // 10. return dealVO
    }

    // 步骤间依赖声明（addStep/run 模式下用于自动排序，execute 模式下用于前置校验）
    protected Set<StepOutputKey<?>> requires();  // 前置依赖，默认空集
    protected StepOutputKey<?> provides();        // 产出声明，默认 null
...
process(R) → createForType() → assembly.build() → getBusinessHelper()
           ...
           → doProcess(T, R) → buildVO() → afterProcess()
           → assembly.complete() → publish(provides, V) → assembly.finish()
```
...
11. `assembly.complete(dealVO, entity, vo)` — 装配单元完成，调用 `before()` 填充测试数据，调用 `finishCurrentUnit()` 执行当前单元的 `entity.finish()` 和异步任务
12. `publish(provides, v)` — 如果 Facade 声明了 `provides()`，自动将 DealVO 发布到 Assembly 的 `StepOutputStore`，供后续步骤通过 `entity.require(key)` 消费
13. `assembly.finish()` — 单模块模式下自动调用，多模块编排时在所有步骤完成后统一调用

**异常处理策略：**

异常处理通过 `ExceptionHandler` SPI 委托，消费方可自定义处理逻辑。默认实现 `DefaultExceptionHandler` 的行为：

```java
try {
    // 正常流程
} catch (SkipException e) {
    return exceptionHandler.onSkip(e, r);       // 跳过当前操作
} catch (DegradeException e) {
    return exceptionHandler.onDegrade(e, r);    // 降级处理
} catch (Exception e) {
    return exceptionHandler.onException(e, r);  // 异常处理
} finally {
    exceptionHandler.onFinally(r);              // 清理回调
}
```
...
```java
/** 使用外部提供的 Assembly 执行 Pipeline（供多模块编排和 TestCaseRunner 使用） */
...
public class BusinessEntity<O extends BusinessHelper<?>> extends UserBusiness {
    ...
    // 步骤输出访问（多模块编排时使用）
    public <E> E require(StepOutputKey<E> key);       // 获取前置步骤输出（必须存在）
    public <E> E getStepOutput(StepOutputKey<E> key); // 获取前置步骤输出（可选）
...
BusinessAssembly 是管道编排的核心，管理执行单元链、步骤间数据传递和测试模式。实现 `AutoCloseable`，支持 try-with-resources 模式：
...
public abstract class BusinessAssembly implements AutoCloseable {
    ...
    // 步骤输出存储 — 步骤间类型安全的数据传递
    private final StepOutputStore outputStore;
    ...
    public void complete(BusinessDealVO<?>, BusinessEntity<?>, UserBusinessVO);  // 完成（含当前单元 finish）

    // 多模块编排 — 命令式（手动控制顺序，执行前校验前置依赖）
    public <V, T, R> V execute(BusinessFacade<V, T, R> facade, R input);

    // 多模块编排 — 声明式（注册步骤，自动拓扑排序执行）
    public <V, T, R> BusinessAssembly addStep(BusinessFacade<V, T, R> facade, R input);
    public BusinessAssembly run();    // 拓扑排序 → 循环依赖检测 → 按序执行 → finish()

    // finish 分层
    // finishCurrentUnit() — 每次 complete 只 finish 当前单元（entity.finish + 异步任务）
    // finish()            — 整条流程线结束时调用一次（保存测试用例等）
    // close()             — AutoCloseable 接口，委托给 finish()
...
| 特性 | `execute()` 命令式编排 | `addStep()` + `run()` 声明式编排 |
|------|----------------------|-------------------------------|
| 执行顺序 | 由调用顺序决定 | 由 requires/provides 依赖关系自动推断 |
| 依赖校验 | 执行前校验 requires 是否已发布 | run() 时统一校验并检测循环依赖 |
| 步骤输出传递 | 支持（publish/require） | 支持（publish/require） |
| 适用场景 | 固定顺序的简单编排 | 复杂依赖关系、顺序可能变化的编排 |
...
### 6.3 StepOutputKey - 类型安全的步骤输出键

`StepOutputKey<T>` 采用 Typesafe Heterogeneous Container（THC）模式，是步骤间数据传递的类型安全键。消费方将其定义为 `public static final` 常量：

```java
public final class StepOutputKey<T> {
    private final String name;      // 唯一标识
    private final Class<T> type;    // 值类型（运行时类型转换）

    public static <T> StepOutputKey<T> of(String name, Class<T> type);
    // equals/hashCode 仅基于 name
}
```

**设计要点：**
- `final class` 防止继承，保证行为确定性
- `equals/hashCode` 仅基于 `name`，不含 `type` — 同名即为同一键
- 消费方通常定义为独立的常量类，解耦于具体模块
...
### 6.4 StepOutputStore - 步骤输出存储

`StepOutputStore` 是类型安全的异构容器，由 Assembly 持有，生命周期跟随 Assembly：

```java
public class StepOutputStore {
    private final Map<StepOutputKey<?>, Object> outputs;

    public <T> void publish(StepOutputKey<T> key, T value);    // 禁止重复 publish
    public <T> T require(StepOutputKey<T> key);                // 不存在则抛异常
    public <T> T get(StepOutputKey<T> key);                    // 不存在返回 null
    public boolean contains(StepOutputKey<?> key);
    public Set<StepOutputKey<?>> publishedKeys();
}
```

**发布时机：** Facade 的 `process(R, Assembly)` 在 `complete()` 之后、返回之前，自动将 DealVO 发布到 `StepOutputStore`（需 Facade 覆写 `provides()` 声明产出）。

**消费方式：** Entity 内通过 `require(key)` 或 `getStepOutput(key)` 获取前置步骤的输出，编译期保证类型安全。

### 6.5 多模块编排

Assembly 支持两种多模块编排方式：

**命令式编排（execute）** — 消费方手动控制执行顺序：

```java
BusinessAssembly assembly = BusinessAssembly.createForType("order-flow");
OrderDealVO order = assembly.execute(orderFacade, orderVO);
PaymentDealVO payment = assembly.execute(paymentFacade, paymentVO);
ShippingDealVO ship = assembly.execute(shippingFacade, shippingVO);
assembly.finish();
```

`execute()` 在进入 `process` 之前校验 Facade 声明的 `requires()` 是否全部已发布，顺序错误时立即报明确错误。

**声明式编排（addStep + run）** — 步骤注册顺序无关，框架根据依赖关系自动排序：

```java
// Facade 声明依赖
public class PaymentFacade extends BusinessFacade<...> {
    @Override protected Set<StepOutputKey<?>> requires() { return Set.of(OutputKeys.ORDER); }
    @Override protected StepOutputKey<PaymentDealVO> provides() { return OutputKeys.PAYMENT; }
}

// 注册顺序无关
BusinessAssembly assembly = BusinessAssembly.createForType("order-flow");
assembly.addStep(shippingFacade, shippingVO)
        .addStep(orderFacade, orderVO)
        .addStep(paymentFacade, paymentVO)
        .run();  // 自动排序: order → payment → shipping → finish()
```

`run()` 的执行流程：
1. 构建 provides → StepRegistration 映射，检测重复 provides 声明
2. 验证所有 requires 的 key 都有对应的 provider
3. Kahn 算法拓扑排序，确定执行顺序
4. 循环依赖检测（排序结果数量 < 步骤数量 = 有环）
5. 按排序顺序依次执行各步骤
6. 调用 `finish()` 完成整条流程线

**try-with-resources 支持** — Assembly 实现 `AutoCloseable`，`close()` 委托给 `finish()`：

```java
try (BusinessAssembly assembly = BusinessAssembly.createForType("order-flow")) {
...
```
         ┌──────┐    ┌──────┐    ┌──────┐    ┌──────┐    ┌──────┐    ┌──────┐
process: │build │───>│ready │───>│doProc│───>│after │───>│compl │───>│publi │
         │      │    │      │    │      │    │Proc  │    │ete   │    │sh    │
         └──────┘    └──────┘    └──────┘    └──────┘    └──────┘    └──────┘
            │           │           │           │           │           │
            ▼           ▼           ▼           ▼           ▼           ▼
       创建Unit     实体初始化   消费方逻辑   @AfterProc  当前Unit     发布步骤输出
       创建Context  Entity就绪  Entity交互   saveDB()    finish       到OutputStore
       创建Helper               Env录制     delRedis()  异步任务
```

**多步骤 finish 分层：**

```
Unit 级别 finishCurrentUnit():       每次 complete 调用，只处理当前 Unit
  ├── entity.finish()                 当前实体的 @AfterProcess(FINISH) 或 helper.finish()
  └── dealVO.executeAllFutureTask()   当前 Unit 的异步任务

Assembly 级别 finish():              整条流程线结束时调用一次
  └── saveTestCase()                  RECORD 模式下保存测试用例
```

```
 ═══════════════════════════════════════════════════════════════════
 RECORD模式额外：                    Unit.finish()      saveTestCase()
...
| **类型安全异构容器(THC)** | `StepOutputKey<T>` + `StepOutputStore` | Bloch《Effective Java》Item 33 模式，泛型键保证容器存取类型安全 |
| **拓扑排序** | `BusinessAssembly.topologicalSort()` | Kahn 算法（BFS）自动解析步骤依赖关系并排序，检测循环依赖 |
...
// StepOutputKey 泛型约束步骤输出的值类型
StepOutputKey<T>    // T 为步骤产出的 DealVO 类型

// Entity 中类型安全地获取前置步骤输出
entity.require(StepOutputKey<OrderDealVO> key) → OrderDealVO  // 编译期保证
```
...
当业务流程涉及多个步骤，且步骤之间存在数据依赖时，通过 `StepOutputKey` 声明步骤间依赖，Assembly 自动或手动编排执行：

```java
// 1. 定义步骤输出键（类型安全的公共常量）
public class OutputKeys {
    public static final StepOutputKey<OrderDealVO> ORDER =
        StepOutputKey.of("order", OrderDealVO.class);
    public static final StepOutputKey<PaymentDealVO> PAYMENT =
        StepOutputKey.of("payment", PaymentDealVO.class);
    public static final StepOutputKey<ShippingDealVO> SHIPPING =
        StepOutputKey.of("shipping", ShippingDealVO.class);
}

// 2. 各 Facade 声明依赖和产出
@Component
public class OrderFacade extends BusinessFacade<OrderDealVO, OrderEntity, OrderVO> {
    @Override public String getAssemblyTypeCode() { return "order-flow"; }
    @Override protected StepOutputKey<OrderDealVO> provides() { return OutputKeys.ORDER; }
    // 无 requires — 第一个步骤
...
public class PaymentFacade extends BusinessFacade<PaymentDealVO, PaymentEntity, PaymentVO> {
    ...
    public void doProcess(PaymentEntity entity, PaymentVO vo) {
        OrderDealVO order = entity.require(OutputKeys.ORDER);  // 编译期类型安全
        // 使用订单信息处理支付...
    }
}

@Component
public class ShippingFacade extends BusinessFacade<ShippingDealVO, ShippingEntity, ShippingVO> {
    @Override public String getAssemblyTypeCode() { return "order-flow"; }
    @Override protected Set<StepOutputKey<?>> requires() {
        return Set.of(OutputKeys.ORDER, OutputKeys.PAYMENT);
    }
    @Override protected StepOutputKey<ShippingDealVO> provides() { return OutputKeys.SHIPPING; }

    @Override
    public void doProcess(ShippingEntity entity, ShippingVO vo) {
        OrderDealVO order = entity.require(OutputKeys.ORDER);
        PaymentDealVO payment = entity.require(OutputKeys.PAYMENT);
        // 使用订单和支付信息处理发货...
    }
}

// 3a. 声明式编排 — 注册顺序无关，自动拓扑排序
BusinessAssembly assembly = BusinessAssembly.createForType("order-flow");
assembly.addStep(shippingFacade, shippingVO)   // 最后执行的步骤先注册
        .addStep(orderFacade, orderVO)          // 第一个执行的步骤后注册
        .addStep(paymentFacade, paymentVO)      // 中间步骤
        .run();  // 自动排序: order → payment → shipping → finish()

// 3b. 命令式编排 — 手动控制顺序（框架校验前置依赖）
try (BusinessAssembly assembly = BusinessAssembly.createForType("order-flow")) {
    OrderDealVO order = assembly.execute(orderFacade, orderVO);
    PaymentDealVO payment = assembly.execute(paymentFacade, paymentVO);
    ShippingDealVO ship = assembly.execute(shippingFacade, shippingVO);
...
// 3c. 不需要步骤依赖的简单场景，require(key) 和 provides()/requires() 可选
OrderDealVO order = orderFacade.process(orderVO);  // 单模块，不涉及步骤间传递
```
...
// 错误：InterruptException("步骤输出键 'xxx' 尚未发布")
// 原因：entity.require(key) 时前置步骤尚未执行或未声明 provides()
// 解决：检查前置步骤的 provides() 声明和执行顺序

// 错误：InterruptException("步骤输出键 'xxx' 被多个步骤声明")
// 原因：多个 Facade 的 provides() 返回相同的 StepOutputKey
// 解决：每个 StepOutputKey 只能由一个步骤声明产出

// 错误：InterruptException("步骤 'xxx' 依赖的输出键 'yyy' 没有任何步骤声明提供")
// 原因：addStep/run 模式下，requires 的 key 没有对应的 provides
// 解决：确保所有依赖的输出键都有步骤声明 provides()

// 错误：InterruptException("检测到循环依赖，涉及步骤: [...]")
// 原因：步骤间的 requires/provides 形成了环
// 解决：检查并消除步骤间的循环依赖关系
```
...
A: `BusinessFacade` 是单个业务步骤的处理门面，定义了 build → process → buildVO → afterProcess → complete → publish 的标准流程。`BusinessAssembly` 是多个步骤的编排器，通过 `execute()` 或 `addStep()`+`run()` 编排多个 Facade，管理步骤间数据传递和测试用例。

**Q: execute() 和 addStep()+run() 如何选择？**

A: `execute()` 适合执行顺序固定且简单的场景，消费方手动控制执行顺序。`addStep()`+`run()` 适合步骤间有复杂依赖关系的场景，步骤注册顺序无关，框架根据 `requires()`/`provides()` 声明自动拓扑排序，并检测循环依赖。两种方式都支持步骤输出传递和依赖校验。

**Q: StepOutputKey 必须声明吗？**

A: 不必须。`requires()` 和 `provides()` 均有安全默认值（空集和 null），不声明时 Facade 的行为与单模块模式完全一致。只有在需要步骤间数据传递或自动排序时才需要声明。

**Q: 步骤输出（StepOutputStore）会参与测试用例录制吗？**

A: 不会。`StepOutputStore` 是运行时状态，不参与持久化。测试用例录制的是 `BusinessEnv` 中的交互记录（`multiBusinessVO`/`multiBusinessEnv`/`multiBusinessDealVO`），步骤输出在回放时会在执行过程中自然重新产生。
...
| **类型安全** | 泛型约束贯穿 Facade → Entity → Helper → VO → StepOutputKey 链路 |
| **自动类型收集** | `ObjectProvider` 自动发现并注册业务类型 |
| **声明式生命周期** | `@AfterProcess` 注解替代继承覆写 |
| **多模块编排** | 声明式（`addStep`+`run` 自动拓扑排序）与命令式（`execute` 手动控制）两种编排方式，步骤间通过 `StepOutputKey` 类型安全传递数据 |
...
进阶：
  ...
  7. 掌握 StepOutputKey + requires/provides 的多模块编排

深入：
  8. 理解 SPI 扩展点体系
  9. 掌握 TestCaseEngine 的测试用例工作流
  10. 了解 CacheInvoke 反射缓存和 LazyFutureTask 异步模型
```
...
12. **StepOutputKey 唯一性**：同一编排流程中，每个 `StepOutputKey` 只能由一个步骤声明 `provides()`，不允许重复发布
13. **requires/provides 对称性**：`addStep`+`run` 模式下，所有 `requires()` 声明的键必须有对应步骤的 `provides()` 声明，否则拓扑排序阶段报错
14. **finish() 幂等性**：`Assembly.finish()` 可安全重复调用，内部状态保证只执行一次。`run()` 和 `close()` 均内部调用 `finish()`
