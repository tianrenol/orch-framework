...
import com.tianrenservice.orch_framework.core.pipeline.StepOutputKey;
...
public class BusinessEntity<O extends BusinessHelper<?>> extends UserBusiness {
    ...
    /**
     * 获取前置步骤的输出（必须存在）
     * 通过 helper → context → assembly → outputStore 链路获取
     *
     * @param key 步骤输出键
     * @return 类型安全的步骤输出
    ...
    public <E> E require(StepOutputKey<E> key) {
        return businessHelper.getBusinessContext().getAssembly().getOutputStore().require(key);
    }

    /**
     * 获取前置步骤的输出（可选，不存在返回 null）
     */
    public <E> E getStepOutput(StepOutputKey<E> key) {
        return businessHelper.getBusinessContext().getAssembly().getOutputStore().get(key);
    }
