...
import com.tianrenservice.orch_framework.core.spi.TypeToken;
...
public class RecordAndReplayHandler {
     ...
     * @return 方法执行结果
     */
    public Object handle(String methodName, Class<?> returnType, Type genericReturnType,
        ...
        switch (mode) {
            ...
            case REGENERATE:
                ...
                if (isVoidMethod) {
                    ...
                    result = castResult(record, returnType, genericReturnType, args);
                }
                break;
            case REPLAY:
                methodRecords = businessEnv.getCovers().get(key);
    ...
    private Object castResult(InteractionRecord record, Class<?> returnType, Type genericReturnType, Object[] args) {
        Object result = record.getResult();
        if (result == null) {
            return null;
        }

        // 1) 泛型变量（如 <V>）→ 从参数中查找 TypeToken 进行泛型反序列化
        if (genericReturnType instanceof TypeVariable<?>) {
            TypeToken<?> typeToken = findTypeToken(args);
            if (typeToken != null) {
                return jsonSerializer.fromJson(jsonSerializer.toJson(result), typeToken.getType());
            }
        }

        // 2) 参数化类型（如 List<OrderDTO>、Map<String,List<String>>、A<B>）→ 直接用完整泛型类型
        if (genericReturnType instanceof ParameterizedType) {
            return jsonSerializer.fromJson(jsonSerializer.toJson(result), genericReturnType);
        }

        // 3) 普通类型（String、int、POJO 等）→ 原有 raw class 逻辑
        Object casted = doCastResult(result, returnType);
        if (casted == null) {
    ...
    private TypeToken<?> findTypeToken(Object[] args) {
        if (args == null) return null;
        for (Object arg : args) {
            if (arg instanceof TypeToken<?> token) {
                return token;
            }
        }
        return null;
